...
class _PurchaseInvoiceScreenState extends State<PurchaseInvoiceScreen> {
  ...
  Future<void> _saveInvoice() async {
    if (_supplierController.text.isEmpty || _invoiceItems.any((i) => i['productId'] == null)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("يرجى ملء كافة البيانات واختيار منتجات")),
      );
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        // إعادة التصفير بدلاً من الـ Pop إذا كنت تريد البقاء في الصفحة
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("إدخال فاتورة شراء جديدة", style: TextStyle(fontFamily: 'Cairo')),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text("تفاصيل المنتجات", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, fontFamily: 'Cairo')),
            const Divider(),
            ...
            ElevatedButton.icon(
              ...
              label: const Text("إضافة منتج آخر", style: TextStyle(fontFamily: 'Cairo')),
        ...
        child: Wrap(
          ...
          children: [
            ...
            _buildDropdown("القسم الرئيسي", _mainCategories, item['mainId'], (val) async {
              ...
              // تصحيح: استخدام isEqualTo بدلاً من التنسيق القديم
              var subs = await _db.collection('subCategory').where('mainId', isEqualTo: val).get();
            ...
            _buildDropdown("القسم الفرعي", item['availableSubCats'], item['subId'], (val) async {
              ...
              // تصحيح: استخدام isEqualTo بدلاً من التنسيق القديم
              var prods = await _db.collection('products').where('subId', isEqualTo: val).get();
  ...
  Widget _buildFooterSection() {
    return Row(
      ...
      children: [
        Text(
          "الإجمالي الكلي: ${_getGrandTotal().toStringAsFixed(2)} ج.م",
          style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.deepPurple, fontFamily: 'Cairo'),
        ),
        SizedBox(
          width: 200,
          height: 50,
          child: ElevatedButton(
            ...
            child: _isSaving
                ...
                : const Text("حفظ الفاتورة", style: TextStyle(color: Colors.white, fontSize: 16, fontFamily: 'Cairo')),
          ),
        )
      ],
    );
