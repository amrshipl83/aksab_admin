...
class ActiveFreeDriversTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // زر عائم للوصول السريع لصفحة الإحالات والنمو
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.data!.docs.isEmpty) {
            return const Center(
              child: Text("لا يوجد مناديب أحرار معتمدين حالياً",
            ...
            child: Column(
              children: [
                ...
                _buildInfoRow(Icons.credit_card, "حد الائتمان الحالي",
                    data['creditLimit']?.toString() ?? "الافتراضي (50)"),
                const Divider(),
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      ...
                      icon: Icon(data['status'] == 'approved' ? Icons.block : Icons.check_circle,
                          size: 18),
                      label: Text(data['status'] == 'approved' ? "إيقاف الحساب" : "تفعيل الحساب",
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
              child: Text(value,
                  style: const TextStyle(fontFamily: 'Cairo', fontSize: 13, color: Colors.blueGrey),
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              dynamic newLimit =
                  controller.text.isEmpty ? null : double.tryParse(controller.text);
  ...
  void _toggleAccountStatus(BuildContext context, String uid, String currentStatus) async {
    ...
    String msg = (newStatus == 'suspended')
        ? "هل أنت متأكد من إيقاف حساب المندوب؟"
        : "هل تريد إعادة تفعيل الحساب؟";
